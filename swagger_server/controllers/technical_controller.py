...
class TechnicalView(MethodView):
    ...
    def get_tech_materials(self):
            internal_process = (None, None)
            function_name = "get_tech_materials"
            response = {}
            status_code = 500
            try:
                if connexion.request.headers:
                    ...
                    message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                    logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                    results = self.technical_use_case.get_tech_materials(internal_transaction_id, external_transaction_id)
                    response["error_code"] = 0
                    response["message"] = "Registros obtenidos correctamente"
                    response["data"] = results
                    end_time = default_timer()
    ...
    def get_auditing_sections(self):
        internal_process = (None, None)
        function_name = "get_auditing_sections"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.technical_use_case.get_auditing_sections(internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = results
                end_time = default_timer()
    ...
    def get_technical_staff(self):
        internal_process = (None, None)
        function_name = "get_technical_staff"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.technical_use_case.get_technical_staff(internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registros obtenidos correctamente"
                response["data"] = results
                end_time = default_timer()
    ...
    def get_history_project(self):
        internal_process = (None, None)
        function_name = "get_history_project"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.technical_use_case.get_history_project(request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
    ...
    def post_location(self):
        ...
        try:
            if connexion.request.is_json:
                ...
                message = f"start request: {function_name}, channel: {body['channel']}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                self.technical_use_case.post_location(body['data'], internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
                response["message"] = "Registro guardado correctamente"
                end_time = default_timer()
                logger.info(f"Fin de la transacción, procesada en : {end_time - start_time} milisegundos",
